...
const timer_store_js_1 = require("./timer-store.js");
...
Returns: Timer ID and target duration. Use this ID with timer_check.`,
    inputSchema: {
        hours: zod_1.z.number().int().min(0).max(23).default(0).describe("Hours (0-23)"),
        minutes: zod_1.z.number().int().min(0).max(59).default(0).describe("Minutes (0-59)"),
        seconds: zod_1.z.number().int().min(0).max(59).default(0).describe("Seconds (0-59)"),
        label: zod_1.z.string().max(100).optional().describe("Optional label for this timer"),
    },
    annotations: {
        readOnlyHint: false,
    ...
    try {
        const timer = (0, timer_store_js_1.startTimer)(hours ?? 0, minutes ?? 0, seconds ?? 0, label);
        const dur = (0, timer_store_js_1.formatDuration)(timer.targetMs);
...
server.registerTool("timer_check", {
...
Returns: Timer status including elapsed/remaining time and expiration status.`,
    inputSchema: {
        id: zod_1.z.string().describe("Timer ID from timer_start"),
    },
    annotations: {
        readOnlyHint: true,
        destructiveHint: false,
    ...
    try {
        const status = (0, timer_store_js_1.checkTimer)(id);
...
server.registerTool("timer_list", {
...
Returns: Array of timer statuses.`,
    ...
    const timers = (0, timer_store_js_1.listTimers)();
    if (timers.length === 0) {
        return {
            content: [{ type: "text", text: "No active timers. Use timer_start to create one." }],
        };
    }
    ...
    return {
        content: [{ type: "text", text: `Timers (${timers.length}):\n${lines.join("\n")}` }],
    };
});
